    ...
    <table class="table table-hover mb-3 text-break">
        ...
        @foreach ($list as $l)
            <tr>
                ...
                <td>{{ $l->created_at }}</td>
                <td>
                    <a href="{{ route('merchant.tianmao.index',['file_id'=>$l->id]) }}" class="btn btn-outline-primary btn-sm">
...
@section('script')
    <x-modal.table-delete id="table-delete" url="{{ route('merchant.tianmao.ajax_destroy_file') }}"/>
@endsection
