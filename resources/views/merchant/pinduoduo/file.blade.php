...
@section('content')
    <div class="mb-3">
        <div class="row g-3">
            <div class="col-auto">
                <a href="{{ route('merchant.pinduoduo.create') }}" class="btn btn-success text-light">
    ...
    <table class="table table-hover mb-3 text-break">
        ...
        @foreach ($list as $l)
            <tr>
                ...
                <td>{{ $l->created_at }}</td>
                <td>
                    <a href="{{ route('merchant.pinduoduo.index',['file_id'=>$l->id]) }}" class="btn btn-outline-primary btn-sm">
...
@section('script')
    <x-modal.table-delete id="table-delete" url="{{ route('merchant.pinduoduo.ajax_destroy_file') }}"/>
@endsection
